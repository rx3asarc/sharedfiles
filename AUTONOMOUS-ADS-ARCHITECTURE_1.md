...
2. **Zero is a claim that must be earned.** A metric is `MEASURED_ZERO` only when a `COMPLETE` coverage record exists for that source, grain and window *and* the entity was eligible to serve; otherwise it is `NO_DATA` or `BROKEN`, and no consumer can read it as zero (enforced by one accessor and a lint test, §4.4).
...
**Growth target and time to get there.** The goal is **100 total store orders/month, then 100/week, then 100/day** (§3.7), all channels counted. Ad spend is governed by revenue, not by hope: **7-day ad spend ≤ 35% of 7-day total store revenue** (Maestro's rule: 350 AUD/day only when the store makes ≥ 1,000 AUD/day), with a 15 AUD/day learning floor (§3.5). So spend can only grow as fast as sales do, and the fastest way to more budget is more revenue from every channel — which is why the system also attacks the surfaces that are free to win: organic rankings, AI answer engines, Shopping free listings and higher conversion rates (§10.5–10.7). The recorded 0.61 Ads conversions/30 d is an artefact of broken tracking, not the sales rate. The system starts from what already sells (§10.0), multiplies it with a creative and landing-page machine that learns by pattern (§12.8–12.10), attacks CPC through Quality Score (§12.9), and expands into localized — not translated — markets when Sweden's demand ceiling binds (§14.7). Earliest timeline: build ≈ 4 weeks, Stage 0 ≥ 10 d, Stage 1 ≥ 14 d, Stage 2 ≥ 21 d → autonomous ad groups, the lander factory and the spend ramp from ≈ day 52; first autonomous campaign ≈ day 73.
...
**The inherited 350 SEK/day (≈ 55.6 AUD) is neither the start nor the goal.** Spend now follows revenue: it may be 55 AUD/day when the store makes ≥ 157 AUD/day, and 350 AUD/day only at ≥ 1,000 AUD/day — and only if paid is also profitable at the margin (D21).
...
|---|---|---|---|
| CTR non-brand Search | 8.81% | pooled historical non-brand Search 189 clicks / 2,145 impressions (§3.3 rows incl. legacy paused data — mined, not surfaced) | 200 |
...
8. Read back via GAQL within 60 s; compare expected fields → `VERIFIED`; mismatch → apply `inverse_json`, `ROLLED_BACK`, incident sev 2.
9. New ads: re-read `policy_summary.approval_status` at +24 h and +72 h.
...
| 5 Offer (bundle, upsell, downsell, discount) | autonomous | via Shopify Admin API (discounts, bundles, and whatever upsell mechanism UNIT 5.13 finds); base product prices never changed; max discount depth config `offer.max_discount = 0.20`; margin check: offer must keep contribution per order ≥ the governor's profit target |
| 6 Email | autonomous | flow variants only affect new flow entrants and are switchable off (reversible). Broadcast campaigns cannot be unsent, so: ≤ 1 per week, engaged segments only, spam-complaint guard (pause all autonomous email if complaint rate > 0.1% or unsubscribe rate > 1% on a send) |
| 7 Organic content | autonomous via the existing article pipeline and its gates | — |
| 8 Markets | autonomous where the store already sells and ships (localized Pages, campaigns, feed); a market needing new tax registration, shipping or a new Shopify Market gets a ready packet, because that is a legal/business setup, not a marketing change | — |
...
Each `ACT` decision stores `predicted_json` (e.g. D01: "term spend next 28 d = 0"; D08: "clicks +x ± y"; D12: hypothesis metrics). At `horizon_days` the `learn` job writes `outcomes` with verdict. Judgement-quality metrics (§20.4) are computed from these. Decisions whose realised effect is confounded by a concurrent change on the same entity within the horizon are `UNMEASURABLE` — the touch lock (G5) minimises this.
...
| Q7 | §4.7 taken literally would hide the system's own kills and experiment weeks | Adsys-paused entities reportable 14 d; E-01 shown as experiment state; legacy never | If Maestro wants zero paused names ever → kills reported by hypothesis id only |
